...
export class InvoiceFormComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.addOrder = new FormGroup({
      ...
      gstin: new FormControl(
        ...
        Validators.compose([
          Validators.required,
          this.customValidator.alphaNumericValidator(),
        ])
      ),
      ...
      tradeDiscountValue: new FormControl(
        '',
        Validators.compose([
          Validators.required,
          this.customValidator.numberValidator(),
      ...
      cashDiscountValue: new FormControl(
        '',
        Validators.compose([
          Validators.required,
          this.customValidator.numberValidator(),
  ...
  createOrder() {
    ...
    if (this.addOrder.valid) {
      this.addOrder.patchValue({
        invoiceDate: this.jsonToString(this.addOrder.get('invoiceDate').value),
      });
      ...
      this.invoiceService.addOrderDetails(this.addOrder.value).subscribe(
        (result) => {
          this.addOrder.reset();
  ...
  onCheckBoxChangeDisableField(list: any[]) {
    ...
    for (let value of list) {
      this.addOrder
        .get(value + '.productSelected')
        .valueChanges.subscribe((selectedProduct) => {
          if (!selectedProduct) {
            if (value == 'product1') {
              this.onLoadHideProd1 = 'unset';
              this.PaddingProd1 = '0px';
              this.productList--;
            }
            if (value == 'product2') {
              this.onLoadHideProd2 = 'unset';
              this.PaddingProd2 = '0px';
              this.productList--;
            }
            if (value == 'product3') {
              this.onLoadHideProd3 = 'unset';
              this.PaddingProd3 = '0px';
              this.productList--;
            }
            this.addOrder.get(value + '.quantity').disable();
            ...
            if (value == 'product1') {
              this.PaddingProd1 = '0.75rem 1.25rem';
              this.productList++;
            }
            if (value == 'product2') {
              this.PaddingProd2 = '0.75rem 1.25rem';
              this.productList++;
            }
            if (value == 'product3') {
              this.PaddingProd3 = '0.75rem 1.25rem';
              this.productList++;
            }
  ...
  handleOtherTransportAndTerms(sourceControl: string, targetControl: string) {
    ...
    this.addOrder.get(sourceControl).valueChanges.subscribe((selectedValue) => {
      if (
        (sourceControl == 'transport' && selectedValue == 'OTHERS') ||
        ...
        if (sourceControl == 'cashDiscount' && !selectedValue)
          this.PaddingCd = '0px';
        if (sourceControl == 'tradeDiscount' && !selectedValue)
          this.PaddingTd = '0px';
        this.addOrder.get(targetControl).disable();
        this.addOrder.get(targetControl).reset();
      }
  ...
  getTotalValue(): number {
    return (
      this.prod1Quantity * this.prod1UnitPrice +
  ...
  getCashDiscountAmt(): number {
    return (
      ((this.getTotalValue() - this.getTradeDiscountAmt()) *
        this.cashDiscount) /
      100
    );
  }
  ...
  getGstAmt(): number {
    return (
      ((this.getTotalValue() -
        this.getTradeDiscountAmt() -
        this.getCashDiscountAmt()) *
        18) /
      100
    );
  }
